...
class OurSCR(BASE):
    ...
    def forward(self, batch, debug=False):
        ...
        i_logits = self.classifier(
            torch.concat([h, pool.unsqueeze(1).expand(b, l, e)], dim=-1)
        )
        ...
        i_logits = self.classifier(
            torch.concat([h__, pool__.unsqueeze(1).expand(b, l, e)], dim=-1)
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        intent_loss = F.binary_cross_entropy_with_logits(
            i_logits, batch["labels"].unsqueeze(1).expand(b, l, i)
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        metric = self.metric.all_compute(
            ...
            n_ints_labels=batch["n_ints_label"],
            pre_train=False,
        )
