...
class OursAIK(BASE):
    def __init__(
        ...
        self.metric = Metric(self.num_classes + 1, num_labels=self.num_classes + 1) # unk 포함
    ...
    def forward(self, batch):
        ...
        attn_probs = self.masked_softmax(weight.transpose(1,2), batch["attention_mask"])  # [batch, intent, seq]
    ...
    def training_step(self, batch, batch_idx):
        ...
        known_intent_loss = F.binary_cross_entropy_with_logits(i_logits, labels[:, :self.num_classes])

        loss = pos_loss * self.l1 + neg_loss * self.l2 + intent_num_loss * self.l3 + known_intent_loss
